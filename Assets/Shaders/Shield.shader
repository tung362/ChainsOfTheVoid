Shader "Custom/Shield"
{
	...
	_Speed("_Speed", Range(0.5,5.0)) = 0.5
		...
		_ViewX("ViewX", Range(-1.0,1.0)) = 0
		_ViewY("ViewY", Range(-1.0,1.0)) = 0
		_ViewZ("ViewZ", Range(-1.0,1.0)) = 0
	}
...
#pragma surface surf BlinnPhongEditor alpha vertex:vert
	...
	void surf(Input IN, inout EditorSurfaceOutput o)
	{
		...
		float dNorm = 1.0 - dot(normalize(float4(float3(IN.viewDir.x + _ViewX, IN.viewDir.y + _ViewY, IN.viewDir.z + _ViewZ), 1.0).xyz), normalize(Fresnel0_1_NoInput.xyz));
		...
		/*o.Albedo = fixed3(IN.viewDir.x, IN.viewDir.y, IN.viewDir.x);
		o.Alpha = 1;*/
	}
	ENDCG
	}
